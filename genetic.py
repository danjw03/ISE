...
def mutate_individual(individual, sensitive_columns, non_sensitive_columns, X_test, mutation_rate=0.1):
    sample_a, sample_b = individual
    ...
    for col in non_sensitive_columns:
        if random.random() < mutation_rate:
            ...
            child_a[col] = unique_values[np.abs(unique_values - (np.clip(child_a[col] + perturbation, min_val, max_val))).argmin()]
            child_b[col] = unique_values[np.abs(unique_values - (np.clip(child_b[col] + perturbation, min_val, max_val))).argmin()]
...
def genetic_algorithm(model, X_test, sensitive_columns, non_sensitive_columns, pop_size=100, generations=50, tournament_size=3, crossover_prob=0.8, mutation_rate=0.1, threshold=0.05):
    population = [generate_individual(X_test, sensitive_columns, non_sensitive_columns) for _ in range(pop_size)]
    ...
    for gen in range(generations):
        ...
        while len(new_population) < pop_size:
            ...
            if random.random() < crossover_prob:
                child = crossover_individuals(parent1, parent2, sensitive_columns, non_sensitive_columns, X_test)
            else:
                child = parent1  # clone if no crossover

            # Mutation
            child = mutate_individual(child, sensitive_columns, non_sensitive_columns, X_test, mutation_rate)
...
def run_fairness_testing(dataset_name):
    ...
    best_ind, best_fit, ga_idi_ratio = genetic_algorithm(
        ...
        pop_size=200, generations=100, tournament_size=3, crossover_prob=0.8,
        mutation_rate=0.3, threshold=0.05
    )
